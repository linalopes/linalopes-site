...
//  End 3D Parallax Slider ---------------------------------------------------------------------------------------------
    ...
    $(".nav-left").on("click", function(e){
        e.preventDefault();
        $body.toggleClass("show-off-screen-left");
    ...
    $outerWrapper.on("click", function(){
        if( $body.hasClass("show-off-screen-left") ){
            $body.removeClass("show-off-screen-left");
        }
    });
    
    $(".nav-right").on("click", function(e){
        e.preventDefault();
        $body.toggleClass("show-off-screen-right");
        $(".scrollbar-inner").stop().animate({
            scrollTop: 0
        }, 800);
    });

    $(".off-screen-right [id]").each(function() {
        $(this).attr("data-scroll-offset", $(this).offset().top);
    });

    $outerWrapper.on("click", function(){
        if( $body.hasClass("show-off-screen-right") ){
            $body.removeClass("show-off-screen-right");
        }
    });
    ...
    $(".page-wrapper").on("mousewheel", function(e) {
        ...
        if( animationFinished === true && delta === 1 && !$body.hasClass("show-off-screen-left") ){
            $(".animate").removeClass("idle");
            if( $(selectedImage).next().length ){
                nextImage = "#" + $(selectedImage).next()[0].id;
                play( nextImage );
